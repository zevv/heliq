...
void Panel::draw(View &view, Simulation &sim, SDL_Renderer *rend, int x, int y, int w, int h)
{
	m_last_w = w;
	m_last_h = h;
	...
	if(m_type == Type::Root) {
		if(m_kids.size() == 1) {
			m_kids[0]->draw(view, sim, rend, x, y, w, h);
		}
	}
	...
	if(m_type == Type::SplitH) {
		...
		for(auto &pk : m_kids) {
			...
			pk->draw(view, sim, rend, kx, y, kw, h);
			kx += kw + 1;
		}
	...
	if(m_type == Type::SplitV) {
		...
		for(auto &pk : m_kids) {
			...
			pk->draw(view, sim, rend, x, ky, w, kh);
			ky += kh + 1;
		}
	}
