...
class WidgetHelix2 : public Widget {

public:
	WidgetHelix2(Widget::Info &info);
	~WidgetHelix2() override;
...
WidgetHelix2::WidgetHelix2(Widget::Info &info)
	: Widget(info)
{
}


WidgetHelix2::~WidgetHelix2()
{
}


void WidgetHelix2::do_save(ConfigWriter &cfg)
{
	m_camera.save(cfg);
...
void WidgetHelix2::do_load(ConfigReader::Node *node)
{
	if(!node) return;
	m_camera.load(node);
...
void WidgetHelix2::do_draw(SimContext &ctx, SDL_Renderer *rend, SDL_Rect &r)
{
...
double WidgetHelix2::compute_max_amp(const psi_t *psi, int n)
{
...
double WidgetHelix2::envelope_value(psi_t psi, double max_amp)
{
...
void WidgetHelix2::mvp_to_float(const mat4 &m, float *out)
{
...
void WidgetHelix2::gl_draw_axis(const mat4 &vp)
{
	float a = m_amplitude;
...
std::tuple<float,float,float> WidgetHelix2::color_for_vert(
...
void WidgetHelix2::gl_draw_surface(const psi_t *psi, double max_amp, int n)
{
...
void WidgetHelix2::gl_draw_helix(const psi_t *psi, double max_amp, int n)
{
...
void WidgetHelix2::gl_draw_envelope(const psi_t *psi, double max_amp, int n,
                                      const mat4 &vp)
{
...
void WidgetHelix2::gl_draw_potentials(const psi_t *pot, int n)
{
	float a = m_amplitude;
...
void WidgetHelix2::gl_draw_potential_marginal(const psi_t *pot, int n)
{
	float a = m_amplitude;
...
void WidgetHelix2::gl_draw_cursor(const GridMeta &gm)
{
	int ax = m_slice.axis;
...
void WidgetHelix2::draw_controls(SimContext &ctx)
{
	auto &state = ctx.state();
...
REGISTER_WIDGET(WidgetHelix2,
	.name = "helix2",
	.description = "3D helix wavefunction viewer (extraction pipeline)",
	.hotkey = ImGuiKey_F5,
);
